...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                ...
                C_proj_bias=None, delta_softplus=True, mask_C = None, checkpoint_lvl=1):
        #xz             ([256, 720, 76])
        #conv1d_weight  ([360, 1(256), 4])
        #conv1d_bias    ([360])
        #x_proj_weight  ([44, 360])
        #delta_proj_weight([360, 12])
        #A              ([360, 16])
        #B              None
        #C              None
        #D              (360)
        ...
        L = xz.shape[-1]# 76
        delta_rank = delta_proj_weight.shape[1]# 12
        d_state = A.shape[-1] * (1 if not A.is_complex() else 2)# 16
        if torch.is_autocast_enabled():
            x_proj_weight = x_proj_weight.to(dtype=torch.get_autocast_gpu_dtype())#([44, 360])
            delta_proj_weight = delta_proj_weight.to(dtype=torch.get_autocast_gpu_dtype())#([360, 12])
        if xz.stride(-1) != 1:
            xz = xz.contiguous()
        conv1d_weight = rearrange(conv1d_weight, "d 1 w -> d w")#([360, 4])
        x, z = xz.chunk(2, dim=1)#([256, 360, 76]) ([256, 360, 76])
        conv1d_bias = conv1d_bias.contiguous() if conv1d_bias is not None else None#([256])
        conv1d_out = causal_conv1d_cuda.causal_conv1d_fwd(x, conv1d_weight, conv1d_bias,None, True)# 卷积后的out ([256, 360, 76])
        # We're being very careful here about the layout, to avoid extra transposes.
        # We want delta to have d as the slowest moving dimension
        # and L as the fastest moving dimension, since those are what the ssm_scan kernel expects.
        x_dbl = F.linear(rearrange(conv1d_out, 'b d l -> (b l) d'), x_proj_weight)  # ([256, 360, 76]) -> (256 * 76, 44)
        delta = rearrange(delta_proj_weight @ x_dbl[:, :delta_rank].t(), "d (b l) -> b d l", l = L)# ([256, 8]) * ([8, 256*360])= ([256, 256*360]) -> ([256, 360, 76])
        ctx.is_variable_B = B is None
        ctx.is_variable_C = C is None
        ctx.B_proj_bias_is_None = B_proj_bias is None
        ctx.C_proj_bias_is_None = C_proj_bias is None
        if B is None:  # variable B
            B = x_dbl[:, delta_rank:delta_rank + d_state]  # (bl dstate) ([256 * 76, 16])  8: 24
            if B_proj_bias is not None:
                B = B + B_proj_bias.to(dtype=B.dtype)
            if not A.is_complex():
                # B = rearrange(B, "(b l) dstate -> b dstate l", l=L).contiguous()
                B = rearrange(B, "(b l) dstate -> b 1 dstate l", l=L).contiguous() #([256, 1, 16, 76])
            else:
        ...
        if C is None:  # variable C
            C = x_dbl[:, -d_state:]  # (bl dstate) ([256*76, 16])  24: 40
            if C_proj_bias is not None:
                C = C + C_proj_bias.to(dtype=C.dtype)
            if not A.is_complex():
                # C = rearrange(C, "(b l) dstate -> b dstate l", l=L).contiguous()
                C = rearrange(C, "(b l) dstate -> b 1 dstate l", l=L).contiguous()#([256, 1, 16, 76])
                # C = C * sup_mask
            else:
        ...
        if mask_C != None:
            sup_mask = rearrange((delta.permute(0, 2, 1) @ A), "b l dstate -> b 1 dstate l", b=delta.shape[0], l=L)#([256, 1, 16, 76]) ([256, 1, 8, 170])
            mask_C = mask_C.squeeze().unsqueeze(1).unsqueeze(1).repeat(1,1,sup_mask.shape[2],1)#([256, 1, 16, 169)
        # # ones_mask = torch.ones_like(delta_a).cuda()#([256, 1, 16, 76]) 1
            mask_C = torch.where(mask_C == 1, mask_C, sup_mask)#([256, 1, 16, 76])
            C = C * mask_C
        out, scan_intermediates, out_z = selective_scan_cuda.fwd(
            conv1d_out, delta, A, B, C, D, z, delta_bias, delta_softplus
            # conv1d_out ([256, 360, 76])
            # delta ([256, 360, 76]) 
            # A (([360, 16])) 
            # B ([256, 1, 16, 76])
            # C ([256, 1, 16, 76])
            ...
            # delta_bias ([360])
            # delta_softplus True
        )#  ([256, 360, 76]) ([256, 360, 1, 32]) ([256, 360, 76])
        ctx.delta_softplus = delta_softplus
...
class MambaInnerFn(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        conv1d_out = causal_conv1d_cuda.causal_conv1d_fwd(x, conv1d_weight, conv1d_bias, None, True)# ([256, 96, 169])  ([96, 4]) 
        # We're being very careful here about the layout, to avoid extra transposes.
        # We want delta to have d as the slowest moving dimension
        # and L as the fastest moving dimension, since those are what the ssm_scan kernel expects.
...
def mamba_inner_fn_no_out_proj(
    xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
    A, B=None, C=None, D=None, delta_bias=None, B_proj_bias=None,
    C_proj_bias=None, delta_softplus=True,mask_C=None,
):
#xz             ([256, 256, 170])
#conv1d_weight  ([256, 256, 4])
#conv1d_bias    ([256])
#x_proj_weight  ([40, 256])
#delta_proj_weight([256, 8])
#A              ([256, 16])
#B              None
#C              None
#D              (256)
...
#delta_softplus True
    ...
    return MambaInnerFnNoOutProj.apply(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                              A, B, C, D, delta_bias, B_proj_bias, C_proj_bias, delta_softplus, mask_C)
