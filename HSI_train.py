...
def experiment():#hyperparams是一个字典形式的配置变量
    ...
    img_src, gt_src, img_tar, gt_tar, _, _, ignored_labels, _, _ = get_rask_dataset(args.rask_name)
    unpad_gt_src = gt_src
    unpad_gt_tar = gt_tar
    ...
    val_dataset = HyperX(img_src_con, val_gt_src_con, shuffle=True,**hyperparams)
    ...
    for epoch in range(args.num_epoch):
        ...
        teacc, kappa, _ = evaluate(model, val_loader, unpad_gt_src, device)
        if best_acc < teacc:
            best_acc = teacc
            best_epoch = epoch
